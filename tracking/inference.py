...
def constructBayesNet(gameState: hunters.GameState):
    ...
    y = Y_RANGE
    mn = MAX_NOISE
    list0 = list(range(0, x + y + mn - 1))
    list1 = list(range(0, x + y + mn - 1))
    for i in range(x):
        for j in range(y):
            holdingList.append((i, j))
    variableDomainsDict[OBS0] = list0
    variableDomainsDict[OBS1] = list1
    variableDomainsDict[PAC] = holdingList
    variableDomainsDict[GHOST1] = holdingList
    variableDomainsDict[GHOST0] = holdingList
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        CPTList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        none = 0
        for e in eliminationOrder:
            CPTList, newFac = joinFactorsByVariable(CPTList, e)
            uncondList = newFac.unconditionedVariables()
            check = len(uncondList)
            if check == 0:
                none += 1
            if not check == 1:
                newFac = eliminate(newFac, e)
                CPTList += [newFac]
        ret = joinFactors(CPTList)
        CPTList = normalize(ret)
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        for i in values:
            t += i
        if not (self == 0 and self.total() == 0):  
            if t == 0:
                for i in keys:
                    self[i] = 0
            else: 
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        gp = ghostPosition
        jp = jailPosition
        mDist = manhattanDistance(pp, gp)
        if gp == jp and noisyDistance is None:
            return 1
        elif gp != jp and noisyDistance is not None:
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in positionList:
            hold = self.getObservationProb(observation, pp, p, jp)
            self.beliefs[p] = self.beliefs[p] * hold
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for i in positionList:
            d2 = self.getPositionDistribution(gameState, i)
            for j in positionList:
                beliefs = self.beliefs
                d1[j] = d1[j] + d2[j] * beliefs[i]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        positionList = self.legalPositions
        for i in range(particles):
            length = len(positionList)
            self.particles.append(positionList[i%length])
        "*** END YOUR CODE HERE ***"
        ...
        for i in particles:
            if i in beliefs.keys():
                if len(particles) > 1:
                    check = 1
                beliefs[i] += 1
            if i not in beliefs.keys():
                beliefs[i] = 1
        hold = beliefs.normalize()
        return beliefs
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        total = 0
        for i in particles:
            obs = self.getObservationProb(observation, pp, i, jp)
            hold[i] = obs + hold[i]
        total = hold.total()
        if total != 0: 
            hold.normalize()
