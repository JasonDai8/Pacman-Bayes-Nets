...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for i in lgpd:
            gpList.append(i.argMax())
        closest = min(gpList, key=lambda gp: abs(self.distancer.getDistance(pp, gp)))
        for i in legal:
            curr = Actions.getSuccessor(pp, i)
            successors.update({i: curr})
        ret = min(legal, key=lambda x: self.distancer.getDistance(closest, successors[x]))
        return ret
        "*** END YOUR CODE HERE ***"
